...
{
    if (connected) {
        /* Save credentials that came from a live portal submission. */
        ...
        if (s_portal_active) {
            /* Portal was running — tear it down and switch to STA-only. */
            http_server_stop();
            dns_server_stop();
            ...
            /* Direct connect path — s_sta_netif already set by wifi_prov_connect. */
        ...
        if (s_portal_active) {
            /* Failed via portal — keep portal running so user can retry. */
            ESP_LOGW(TAG, "Connection failed – portal remains active");
        } else {
            /* Failed via wifi_prov_connect — clean up WiFi; portal is NOT started.
               The caller decides what to do next (e.g. call wifi_prov_start()). */
            ESP_LOGW(TAG, "Connection failed – cleaning up WiFi");
            stop_wifi();
        }

        if (s_config.on_connect_failed) {
            s_config.on_connect_failed();
        }
    }
}
...
esp_err_t wifi_prov_start(const wifi_prov_config_t *config)
{
    ...
    /* If WiFi is already running (e.g. a failed wifi_prov_connect left residue),
       stop it cleanly before starting the portal. */
    wifi_mode_t mode;
    ...
    /* Re-register credential event (safe if already registered). */
...
esp_err_t wifi_prov_connect(void)
{
    ...
    /* Stop any leftover WiFi state before starting fresh. */
    wifi_mode_t mode;
    if (esp_wifi_get_mode(&mode) == ESP_OK) {
        stop_wifi();
    }

    /* Initialize WiFi in STA-only mode; portal is NOT started. */
    wifi_init_config_t wifi_init = WIFI_INIT_CONFIG_DEFAULT();
    ESP_ERROR_CHECK(esp_wifi_init(&wifi_init));
